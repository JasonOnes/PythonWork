
from flask import Flask, render_template, request, redirect, escape
from vsearch import search_word

app = Flask(__name__)


def log_request(req: 'flask_request', res: str) -> None:#req = request object, res = result
    with open('vsearch.log', 'a') as log:
        print(req.form, req.remote_addr, req.user_agent, res, file=log, sep='|')#breaks info with ||

@app.route('/search4', methods=['Post'])#URL
def do_the_search()-> 'html':
    phrase = request.form['phrase']
    letters = request.form['letters']
    title = 'Here\'s your results buddy!'
    results = str(search_word(phrase, letters))
    log_request(request, results)
    return render_template('results.html', 
                           the_phrase=phrase,
                           the_letters=letters,
                           the_title=title,
                           the_results=results,)
                        #    assigning python variables to their jinja2 counterparts
                # corresponding to the 4 arg values in results.html

#rather than redirect we can simply add more routes to try 
@app.route('/')
@app.route('/entry')
@app.route('/taco')
def entry_page() -> 'html':
    return render_template('entry.html',
                           the_title='Welcome to the Dumbest letter search online!')

@app.route('/viewlog')
def view_the_log() ->str:
    contents = []
    with open('vsearch.log', 'r') as log:
        for line in log:
            contents.append([])
            for item in line.split('|'):
                contents[-1].append(escape(item))
    titles = ('Form Data', 'Remote_addr', 'User_agent', 'Results',)
    return render_template('viewlog.html',
                           the_title='View Log',
                           the_row_titles=titles,
                           the_data=contents,)
    #return str(contents)
        #contents = log.readlines()
        #return escape(''.join(contents))
    

if __name__ == '__main__':
    app.run(debug=True)

